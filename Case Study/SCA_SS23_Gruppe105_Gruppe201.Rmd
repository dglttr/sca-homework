...
materials %>% group_by(Month=as.yearmon(EinAusDat)) %>% summarize(cnt=n()) %>% mutate(future=ifelse(Month > as.yearmon("2023-06-12"), "Future", "Past")) %>%
  ...
  geom_smooth(data=materials %>% filter(EinAusDat <= "2023-06-12") %>% group_by(Month=as.yearmon(EinAusDat)) %>% summarize(cnt=n()) %>% mutate(future=ifelse(Month > as.yearmon("2023-06-12"), "Future", "Past")), method=lm, se=FALSE)+
   ylim(0, NA)
```
...
```{r}
# Treemap
materials_agg <- materials %>% group_by(`Artikel-Bezeichnung`) %>% count()

treemap(materials_agg, index="Artikel-Bezeichnung", vSize="n")

materials %>% group_by(`Artikel-Bezeichnung`) %>% count() %>% arrange(desc(n)) %>% mutate(`Artikel-Bezeichnung` = substring(`Artikel-Bezeichnung`, 0, 50)) %>%
  ggplot(aes(area=n, fill=`Artikel-Bezeichnung`, label=`Artikel-Bezeichnung`)) +
...
```{r}
# Kurve
materials %>% group_by(`Artikel-Bezeichnung`) %>% count() %>% arrange(desc(n)) %>% mutate(`Artikel-Bezeichnung` = substring(`Artikel-Bezeichnung`, 0, 50)) %>% ggplot(aes(x=reorder(`Artikel-Bezeichnung`, -n), y=n))+geom_col()+labs(y="Anzahl Transaktionen")
```
...
```{r}
# Top 15 Projekte
materials_b %>% group_by(PjInfo, klasse) %>% count() %>% arrange(desc(n)) %>% mutate(PjInfo=substring(PjInfo, 0, 40)) %>% head(15) %>% ggplot(aes(x=n, y=reorder(PjInfo, n), fill=klasse))+
  geom_col()+
...
materials %>% filter(Menge <0) %>% group_by(klasse) %>% summarize(Nachfrage=sum(-Menge)/10**6) %>%
  ...
  scale_fill_manual(values=c("Handel" = "#2A2AD5",
                             ...
                             "Einkauf" = "#00FFFF", 
                             "Anderes" = "grey"))+
  labs(x="Geschäftsbereich", y="Nachfrage") +
  theme(legend.position = "none")
```
...
materials %>% group_by(mon=as.yearmon(EinAusDat)) %>% summarize(anz_mat=n_distinct(`Artikel-Bezeichnung`)) %>% ggplot(aes(x=mon, y=anz_mat))+geom_col()
```
...
picks %>% filter(lot_size != 0) %>%
  ...
  geom_histogram()+
  geom_vline(data=picks %>% filter(lot_size != 0) %>% group_by(`Artikel-Bezeichnung`) %>% summarize(mean_lot_size=mean(lot_size, na.rm=TRUE)), aes(xintercept=mean_lot_size), color="blue", linetype="dashed", size=0.75) +
