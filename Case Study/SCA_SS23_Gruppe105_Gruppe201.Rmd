...
materials_b %>% filter(Menge < 0) %>% group_by(`Artikel-Bezeichnung`, EinAusDat, klasse) %>% summarize(Nachfrage=sum(-Menge)) %>%
  ggplot(aes(x=EinAusDat,y=Nachfrage,group=klasse, color=klasse)) +
    ...
    facet_wrap(~`Artikel-Bezeichnung`, scales = 'free_y')
```
## Fehleranalyse der Bestandszahlen 
```{r}
bestand <- materials_b %>% group_by(`Artikel-Bezeichnung`) %>% mutate(bestand_lagged = Menge + Bestand) %>% mutate(bestand_aprox = lag(bestand_lagged)) 
bestand %>% summarise(falscher_lagerbestand_prozent = sum(bestand_lagged == bestand_aprox, na.rm = TRUE)/n())
bestand %>% ggplot(aes(x =EinAusDat)) +
...
monthly_mean <- weekly_Menge %>%  group_by(`Artikel-Bezeichnung`, month = floor_date(week, 'month')) %>% mutate(mean_Menge= mean(Menge_per_week))
...
## Abruflosgrößen vs. Bestell-Losgröße
```{r}
picks %>% filter(ArtikelNr == 29523, ) %>% 
  ggplot() +
  geom_line(aes(x=EinAusDat,y=picks, col= 'Abruflosgrößen')) +
  geom_line(aes(x=EinAusDat,y=lot_size, col='Bestell-Losgröße'))
```
...
## Abruflosgrößen vs. Bestell-Losgröße nach Artikel
```{r}
...
picks %>% filter(ArtikelNr == 6203,lot_size !=0  ) %>%
  ...
  labs(x="Losgröße", y="Anzahl Bestellungen")
```
# Median der Bestell-Losgrößen nach Artikel
```{r}
picks %>% filter(lot_size !=0) %>%  group_by(`Artikel-Bezeichnung`) %>% summarise(median_lot = median(lot_size))
```

#Verlauf der Abruflosgrößen nach Artikel
```{r}
picks %>% filter(picks !=0) %>% 
  ggplot()+
  geom_line(aes(x= EinAusDat, y = picks, color= `Artikel-Bezeichnung`))+
  theme(legend.position = 'none')+
   facet_wrap(~`Artikel-Bezeichnung`, nrow=2, scales="free") 

```

# Verteilung der Abruflosgrößen nach Artikel
```{r}
...
## Zeitliche Verteilung der Warenein- und ausgänge
```{r}
picks1 <- picks %>%  mutate(picks = replace(picks, picks==0, NA), lot_size = replace(lot_size, lot_size==0, NA))
```

```{r}
picks_diff <- picks1 %>% group_by(`Artikel-Bezeichnung`,is_pick = Menge<0) %>% summarise(days_diff = time_length(interval(lag(EinAusDat), EinAusDat), unit="days")) %>%  mutate(type = if_else(is_pick, 'Warenausgang', 'Wareneinnahmen'))

picks_diff %>% ggplot(aes(x = days_diff, color = type)) +
 geom_boxplot() +
  
 facet_wrap(~`Artikel-Bezeichnung`, nrow=2, scales="free_x") 
```
Wir sehen dass sich Bestellverhalten zwischen Materialien stark unterscheiden, so werden eienige Materialien in deutlich kurzeren Abständen wieder bestellt, wie bei den PVC-Röhren. 

```{r}
picks_diff %>%  group_by(`Artikel-Bezeichnung`,is_pick) %>% drop_na() %>%  summarise(avg_days = mean(days_diff), max_days = max(days_diff), min_days = min(days_diff), sd = sd(days_diff) ) 
```
...
weekly_meanpick <- weekly_picks %>%  group_by(`Artikel-Bezeichnung`) %>%
...
weekly_picks %>% 
  ggplot() +
  geom_line(aes(x=week ,y=picks_per_week, group=`Artikel-Bezeichnung`, col= `Artikel-Bezeichnung`))+
  xlab('Datum')+
  ylab('Abruflosgröße')
```
```{r}
...
weekly_avg_picks %>% 
  ggplot() +
  geom_line(aes(x=week ,y=avgpicks_per_week, group=`Artikel-Bezeichnung`   , col=`Artikel-Bezeichnung`   ))
```
...
materials_b_tägl_menge %>% ggplot() +
  ...
  facet_wrap(~`Artikel-Bezeichnung`, scales="free_y", nrow=3)+
    theme(strip.text.x = element_text(hjust=0.0))+
    labs(x="Zeit", y="Tägliche Nachfrage")
...
## Bestellpolitik
```{r}
perzentil_nachfrage <- 0.7
perzentil_wbz <- 0.0  # 0 = min(wbz)
...
bestellpolitik <- bestellpolitik %>% mutate(Losgröße.Empfehlung = as.numeric(Nachfrage_quantil*WBZ_quantil)) 
bestellpolitik
```
...
materials_b <- materials_b %>% select(-Baseline)
...
for (mat in materials_group_b) {
  ...
  wbz_quantil <- as.numeric(bestellpolitik[bestellpolitik$`Artikel-Bezeichnung`==artikel, "WBZ_quantil"])
  ...
  # Bestellzeitpunkte und -mengen berechnen
  # Variablen für Loop
  ...
  cat("\nAnzahl Stock-outs MBG = ", nrow(materials_b %>% filter(Bestand < 0 & lag(Bestand) >=0)))
  
  # DataFrame mit MBG Performance zusammenführen
  vergleich <- rbind(ges %>% mutate(Bestellpolitik="Empfehlung"),
                     materials_b %>% filter(`Artikel-Bezeichnung`==artikel) %>%
                       mutate(cum_sum=Bestand, Bestellpolitik="MBG") # Bestand statt cumsum der Menge, um richtige Anfangswerte zu haben
