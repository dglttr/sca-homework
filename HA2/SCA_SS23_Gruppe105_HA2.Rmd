...
```{r}
Demand <- transactions %>% arrange(Date) %>%  group_by(month = floor_date(Date, 'month'), region) %>% summarise(sum_sales= sum(Sales))
Demand <- Demand %>% mutate(Period = cur_group_id()) %>% subset(select = -c(month))
head(Demand) %>%  kable()
#Demand[order(Demand$region), ]
```
...
Demand_wide <- pivot_wider(data = Demand, id_cols = "Period", 
                               names_from = "region", values_from = "sum_sales")
```

## Aufgabe 3
```{r}
ts_Japan = ts(Demand_wide$Japan, frequency = 12)

ts_Peking = ts(Demand_wide$Peking, frequency = 12)
ts_Phlppn = ts(Demand_wide$Phlppn, frequency = 12)
ts_Shangh = ts(Demand_wide$Shangh, frequency = 12)
ts_Skorea = ts(Demand_wide$Skorea, frequency = 12)
```
...
Man kann an der Zeitreihe eine Sesonalität erkennen, es gibt einen jedoch keinen Trend.
