...
## Löschen der Spalten  'Year' und 'Month, da sie nicht mehr nötig
...
## Löschen der Spalten 'Year', 'Month' und 'Day' , da sie nicht mehr nötig
services <- subset(services, select=-c(Year, Month, Day))

# Analog für 'transactions'
# Erstellen einer 'Date' Spalte und Löschen der individuellen Variablen 'Year', 'Month' und 'Day' 
...
transactions_regions %>% kable(row.names = FALSE)
```
## Aufgabe 4
...
# Selektion der einzigartigen 'vendor' für Warehousing-DL und Shipping-DL
...
service_vendors %>% kable(row.names = FALSE)
```
...
# Selektion der einzigartigen 'supermarkets' 
supermarkets <- unique(supermarkets)

# Selektion aller existierenden Supermärkte in 'Shangh','Peking' und 'Skorea'
supermarkets <- subset(supermarkets, region %in% c('Shangh','Peking', 'Skorea'))

supermarkets %>% kable(row.names = FALSE)
```
# Marktübersicht
...
```
Der Monat mit dem höchsten durchschnittlichen Absatz von etwa 18 Stück ist der Oktober 2019.
...
```{r}
# Aggregation der monatlichen Sales Daten der Gruppe 105
...
# Aggregation der monatlichen Kosten Daten der Gruppe 105
...
# Gesamtprofit nur Jahr 2020 anschauen
...
# Kosten verspätete und pünktliche Lieferungen pro Region
shipping_byregion <- summarise(group_by(shipping_data105, region, late), total_cost=sum(cost))
# Visualisierung der Kosten verspätete und pünktliche Lieferungen pro Region
...
shipping_vendor_quality <- services18_22 %>%
  ...
  # Berechunung durschnittlichen IFR, durschnittlichen Kosten pro tatsächlich abgefertigt Menge und Anzahl der Lieferungen
...
## Aufgabe 17
```{r}
# Visualisierung der KPI für Shipping-Dienstleister
...
In der ersten Phase betrachten wir das wirtschaftliche Problem, noch ohne Blick auf die Daten. Dazu machen wir eine Lageanalyse, sprechen mit relevanten internen Stakeholdern wie der Logistik- oder Vertriebsabteilung, und legen abgestimmte Analyseziele fest. Ziele könnten etwa sein:
...
Insgesamt zeigt sich aber auch, dass weitere Analysen nötig sind. Zum Beispiel sollte geprüft werden, ob bestimmte LDL besonders in bestimmten Regionen agieren (ist z. B. Bange+Hammer Shipping in Regionen aktiv, die systematisch bessere OTD haben als andere?).
