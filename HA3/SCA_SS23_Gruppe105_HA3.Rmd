...
ldl_ifr_warehousing %>%
  ...
  summarize(IFR=mean(IFR)) %>%
  arrange(desc(IFR)) %>%
  kable(caption="LDL nach IFR, über Regionen und Perioden hinweg", digits=3)
```

Der insgesamt beste LDL über die Regionen und Perioden hinweg ist Flying Mercury Warehousing mit einer IFR von 83,7%.
...
ldl_ifr_warehousing %>%
  ...
  arrange(IFR) %>%
  kable(caption="Aufsteigend sortierte IFR-Werte von Flying Mercury Warehousing in der Region Shanghai", digits=3)
```

Der schlechteste IFR-Wert von Flying Mercury Warehousing in der Region Shanghai beträgt 78,4% und war in der Periode 202205 (Mai 2022).
