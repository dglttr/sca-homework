...
ldl_ifr_shipping %>%
  ...
  head(1) %>%
  kable(caption="Niedrigster OTD-Wert und entsprechender LDL in der Region Japan im April 2019", digits=3)
```
...
data.frame(MAE=mean(abs(test_data$IFR - predictions_test)),
           MAPE=mean(abs((as.numeric(test_data$IFR - predictions_test)/as.numeric(test_data$IFR))*100))) %>% kable(caption="MAE und MAPE des Modells auf den Testdaten", digits=4)
```
